...
def summarize_segment(segment, api_key):
...
# 定义数据预处理函数
def preprocess_data(corpus_path, output_dir, model, tokenizer,api_key, n=500):
    # 确保输出目录存在
    ...
    for filename in os.listdir(corpus_path):
        if filename.endswith(".txt") and filename not in processed_files:
            ...
            for segment in segments:
                if api_key:
                    segment = summarize_segment(segment, api_key)
                else:
                    summary = summarize_segment(segment, model, tokenizer)
                # 将段落按句号分割为句子
...
def summarize_segment(segment, model, tokenizer):
 prompt = "\n你是一个熟读各类小说的专家，请将以上小说内容确定其背景并且用一句话总结故事情节不要进行续写。严格按照以下格式回复：背景：XXXX剧情梗概：XXXX(一句话)。注意：只需提供背景和剧情梗概，不要续写正文。"
 prompt = '小说内容如下：' + segment + prompt
...
def main():
    ...
    parser.add_argument("--api_key", type=str, required=True, help="Ollama API key")
    ...
    # 预处理数据
    preprocess_data(args.corpus_path, args.output_dir,model,tokenizer,args.api_key,args.n)
